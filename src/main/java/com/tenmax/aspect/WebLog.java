...
public class WebLog {
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        logger.info("METHOD    : {}", request.getMethod());     //http请求类型
        logger.info("URI       : {}", request.getRequestURI());
        logger.info("PARAMS    : {}", request.getQueryString());//get方法参数或URL中携带所有参数
        logger.info("REMOTE_IP : {}", request.getRemoteAddr()); //客户端IP
        logger.info("FUNCTION  : {}", joinPoint.getSignature().getDeclaringTypeName() + "  " + joinPoint.getSignature().getName()+" ");//调用方法
        Object[] argsObj = joinPoint.getArgs();
        logger.info("ARGS      : {}", (argsObj.length > 0 ? (argsObj.length > 1 ? Arrays.toString(argsObj) : argsObj[0]) : ""));//方法实际接收的参数
...
//            paramSB.append("  ");
//        }
//        logger.info("PARAMS : " + paramSB.toString());
        logger.info("===========================REQ===========================");
    }
    ...
    public Object doAround(ProceedingJoinPoint pjp) throws Throwable {
        ...
        logger.info("FUNCTION : {}", pjp.getSignature().getDeclaringTypeName());
